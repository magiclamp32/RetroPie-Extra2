...
rp_module_id="solarus1.6.0"
rp_module_desc="solarus - An Open Source Zelda LttP Engine. v1.6.0 includes a new renderer which doesn't work with raspberry pi yet. There is todo stuff in the upstream code regarding rpi"
...
function sources_solarus1.6.0() {
    ...
    downloadAndExtract "https://gitlab.com/solarus-games/zelda-xd2-mercuris-chess/-/archive/v1.1.0/zelda-xd2-mercuris-chess-v1.1.0.tar.gz" "$md_build"
}

function build_solarus1.6.0() {
    mkdir build
    cd build
    ...
    md_ret_require=(
        ...
        "$md_build/zelda-xd2-mercuris-chess-v1.1.0/data.solarus"
    )
}

function install_solarus1.6.0() {
    cd build
    make install
    cd ../zsdx-1.12.0/
    make install
...
function configure_solarus1.6.0() {
