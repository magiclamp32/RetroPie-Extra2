...
rp_module_id="lr-mess-neocdz"
rp_module_name="SNK Neo Geo CD"
rp_module_ext=".chd"
...
neocdz.zip (must contain neocd.bin and 000-lo.lo).\n\n
...
function depends_lr-mess-neocdz() {
...
function sources_lr-mess-neocdz() {
	true
}

function build_lr-mess-neocdz() {
	true
}

function install_lr-mess-neocdz() {
	true
}

function configure_lr-mess-neocdz() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	local _retroarch_bin="$rootdir/emulators/retroarch/bin/retroarch"
	local _system="neocdz"
	...
	addEmulator 1 "$md_id" "$_system" "$_script $_retroarch_bin $_mess $_config $_system $biosdir -cdrm %ROM%"
